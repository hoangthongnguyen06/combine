...
def update_topics_new_from_api(api_url, headers_platform=None, app=None):
    with app.app_context():
        if Config.PLATFORM_API_URL in api_url:
            try:
                response = requests.post(
                    api_url, json=payload_platform, headers=headers_platform, verify=False)
                if response.status_code == 200:
                    ...
                    for item in api_data:
                        ...
                        if end_at:
                            end_at_datetime = datetime.strptime(
                                end_at, "%Y/%m/%d %H:%M:%S")
                            if end_at_datetime < datetime.now():
                                topic_data["status"] = "deactive"
                            else:
                                topic_data["status"] = "active"
                        topic = Topic_new.query.filter_by(
                            uid=topic_data['uid']).first()
                        if topic:
                    ...
                    print(
                        f"PLATFORM: Failed to fetch objects: {response.status_code}")
            except requests.exceptions.RequestException as e:
                print(f"PLATFORM: An error occurred with the request: {e}")
            except Exception as e:
                print(f"PLATFORM: An unexpected error occurred: {e}")
        # if Config.SPYDER_API_URL in api_url:
        #     print("spider")
        #     try:
        #         response = requests.post(
        #             api_url, json=payload_platform, headers=headers_spyder, verify=False)
        #         if response.status_code == 200:
        ...
        #         else:
        #             print(
        #                 f"SPYDER: Failed to fetch objects: {response.status_code}")
            # except requests.exceptions.RequestException as e:
